    ...
    return (
      ...
      <View style={{padding:10, flexDirection:'row', justifyContent:'space-between', bottom:-8}}>
          <View>
              ...
              <Text style={{fontFamily:'Poppins-SemiBold', fontSize:14, }}>{data.username}</Text>
          </View>
          <View>
          <TouchableOpacity onPress={() => navigation.navigate("Profile1")}>
              <View>
                  {profileImage ? (
                    <Image style={{ width: 60, height: 60, borderRadius: 20 }} source={{ uri: profileImage }} />
                  ) : (
                    <Image style={{ width: 60, height: 60, borderRadius: 20 }} source={DefaultPorfile} />
                  )}
              </View>
          </TouchableOpacity>
          </View>
